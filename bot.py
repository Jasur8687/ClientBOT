...
async def cmd_start(message: Message):
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📘 Инструкция", url="https://t.me/xakermines")],
        [InlineKeyboardButton(text="✅ Я подписался", callback_data="subscribed")]
    ])
    await message.answer(
        "Добро пожаловать в Hacker Mines! Перед началом подпишись на канал и прочитай инструкцию:",
...
async def subscribed_handler(callback_query: CallbackQuery):
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🚀 Пройти регистрацию", url="https://1wrjmw.com/v3/lucky-jet-updated?p=vayp")]
    ])
